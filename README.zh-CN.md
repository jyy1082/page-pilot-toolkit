...
书签的地址锁定在某个具体版本（`page-pilot-toolkit@0.3.0`，以及 `toolkit.js` 内部锁定的 page-pilot / page-pilot-recorder 版本号）。哪怕这个仓库之后有改动，已经装好的书签也会一直保持原来的行为不变——想用新版本，重新打开安装页面，再拖一次（更新过的）按钮就行。
